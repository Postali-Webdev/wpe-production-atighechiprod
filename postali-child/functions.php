 ...
 */
	...
	function postali_child() {
		...
		wp_register_style( 'google-fonts', 'https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100..900;1,100..900&display=swap', array() );
		wp_enqueue_style('google-fonts');
        wp_register_style( 'icomoon', 'https://cdn.icomoon.io/152819/AtighechiLawGroup/style.css?j511ob', array() );
		...
		// Smooth Scroll
		wp_register_script('smooth-scroll-min', get_stylesheet_directory_uri() . '/assets/js/smooth-scroll.min.js',array('jquery'), null, true); 
	...
	function postali_child_register_nav_menus() {
		register_nav_menus(
			array(
				...
				'footer-pa-nav' => __( 'Footer Practice Areas', 'postali' ),
                'footer-ql-nav' => __( 'Footer Quick Links', 'postali' ),
			)
		);
	}
    ...
    function testimonials_query( $the_query ) {
        if ( ! is_admin() && $the_query->is_main_query() && is_post_type_archive( 'testimonials' ) ) {
            // Display 12 posts for a custom post type called 'movie'
            $the_query->set( 'posts_per_page', 9 );
            return;
        }
    }
    add_action( 'pre_get_posts', 'testimonials_query', 1 );
	...
	function mainmenu_navsearch($items, $args) {
		if ($args->theme_location == 'header-nav') {
			ob_start();
			?>
			<li class="menu-item menu-item-search search-holder mobile">
